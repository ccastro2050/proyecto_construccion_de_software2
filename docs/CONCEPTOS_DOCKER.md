...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8043
EXPOSE 8043
...
| `EXPOSE 8043` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8043"]
            POSTGRES[("SERVIDOR DE BASE DE DATOS<br/>PostgreSQL · contenedor postgres<br/>hostname: postgres · escucha en 5432")]
        end
    end
    NAV -->|"localhost:8043"| APIFACTURAS
    APIFACTURAS -->|"postgres:5432 (DNS de Docker)"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15443"| POSTGRES
```
...
```yaml
  postgres:
    ...
    ports:
      - "15443:5432"                 # "puerto en su PC : puerto interno"
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8043`, `localhost:15443`); entre contenedores, nombres de
...
docker network create proyecto_construccion_de_software2_default

# 2. postgres
docker run -d --name postgres --network proyecto_construccion_de_software2_default --restart unless-stopped `
  ...
  -p 15443:5432 postgres:16-alpine
...
docker run -d --name api-facturas --network proyecto_construccion_de_software2_default --restart unless-stopped `
...
```yaml
URL_API: http://api-facturas:8043
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8043` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8043` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8043:8043`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8043`, el front
...
```powershell
docker network inspect proyecto_construccion_de_software2_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_construccion_de_software2` produce
la red `proyecto_construccion_de_software2_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_construccion_de_software2-api-facturas-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_construccion_de_software2_default has active endpoints (name:"proyecto_construccion_de_software2-api-facturas-1" …)
```
